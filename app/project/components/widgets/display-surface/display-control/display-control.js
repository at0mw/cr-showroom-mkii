...
const displayControlInstanceModule = (id, elementIds) => {
    'use strict';    

    // BEGIN::CHANGEAREA - your initialization code for each instance of widget goes here  
    // console.log(`displayControl-widget displayControlInstanceModule("${id}", [${elementIds}])`);
    ...
    const cleanup = () => {
        // console.log(`displayControl-widget displayControlInstanceModule cleanup("${id}")`);
    };
...
const displayControlModule = (() => {
     ...
     */
    function onInit() {
       serviceModule.addEmulatorScenarioNoControlSystem("./app/project/components/widgets/display-surface/display-control/display-control-emulator.json");
       // Uncomment the below line and comment the above to load the emulator all the time.
       // serviceModule.addEmulatorScenario("./app/project/components/widgets/display-surface/display-control/display-control-emulator.json");
    }

    /**
     * private method for widget class creation
     */
    let loadedSubId = CrComLib.subscribeState('o', 'ch5-import-htmlsnippet:displayControl-import-widget', (value) => {
        if (value['loaded']) {
            onInit();
            setTimeout(() => {
                CrComLib.unsubscribeState('o', 'ch5-import-htmlsnippet:displayControl-import-page', loadedSubId);
                loadedSubId = '';
            });
        }
    });
     ...
     */
    CrComLib.subscribeState('o', 'ch5-template:display-control-widget', (value) => {
        if (value['loaded'] !== undefined && value['id'] !== undefined) {
            if (value.loaded) {
                widgetInstances[value.id] = displayControlInstanceModule(value.id, value['elementIds']);
            }
            else {
