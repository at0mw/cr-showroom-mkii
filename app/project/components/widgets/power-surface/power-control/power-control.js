...
const powerControlInstanceModule = (id, elementIds) => {
    'use strict';    

    // BEGIN::CHANGEAREA - your initialization code for each instance of widget goes here  
    // console.log(`powerControl-widget powerControlInstanceModule("${id}", [${elementIds}])`);
    ...
    const cleanup = () => {
        // console.log(`powerControl-widget powerControlInstanceModule cleanup("${id}")`);
    };
...
const powerControlModule = (() => {
     ...
     */
    function onInit() {
       serviceModule.addEmulatorScenarioNoControlSystem("./app/project/components/widgets/power-surface/power-control/power-control-emulator.json");
       // Uncomment the below line and comment the above to load the emulator all the time.
       // serviceModule.addEmulatorScenario("./app/project/components/widgets/power-surface/power-control/power-control-emulator.json");
    }

    /**
     * private method for widget class creation
     */
    let loadedSubId = CrComLib.subscribeState('o', 'ch5-import-htmlsnippet:powerControl-import-widget', (value) => {
        if (value['loaded']) {
            onInit();
            setTimeout(() => {
                CrComLib.unsubscribeState('o', 'ch5-import-htmlsnippet:powerControl-import-page', loadedSubId);
                loadedSubId = '';
            });
        }
    });
     ...
     */
    CrComLib.subscribeState('o', 'ch5-template:power-control-widget', (value) => {
        if (value['loaded'] !== undefined && value['id'] !== undefined) {
            if (value.loaded) {
                widgetInstances[value.id] = powerControlInstanceModule(value.id, value['elementIds']);
            }
            else {
