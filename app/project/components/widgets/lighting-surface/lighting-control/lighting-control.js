...
const lightingControlInstanceModule = (id, elementIds) => {
    'use strict';    

    // BEGIN::CHANGEAREA - your initialization code for each instance of widget goes here  
    // console.log(`lightingControl-widget lightingControlInstanceModule("${id}", [${elementIds}])`);
    ...
    const cleanup = () => {
        // console.log(`lightingControl-widget lightingControlInstanceModule cleanup("${id}")`);
    };
...
const lightingControlModule = (() => {
     ...
     */
    function onInit() {
       serviceModule.addEmulatorScenarioNoControlSystem("./app/project/components/widgets/lighting-control/lighting-control-emulator.json");
       // Uncomment the below line and comment the above to load the emulator all the time.
       // serviceModule.addEmulatorScenario("./app/project/components/widgets/lighting-control/lighting-control-emulator.json");
    }

    /**
     * private method for widget class creation
     */
    let loadedSubId = CrComLib.subscribeState('o', 'ch5-import-htmlsnippet:lightingControl-import-widget', (value) => {
        if (value['loaded']) {
            onInit();
            setTimeout(() => {
                CrComLib.unsubscribeState('o', 'ch5-import-htmlsnippet:lightingControl-import-page', loadedSubId);
                loadedSubId = '';
            });
        }
    });
     ...
     */
    CrComLib.subscribeState('o', 'ch5-template:lighting-control-widget', (value) => {
        if (value['loaded'] !== undefined && value['id'] !== undefined) {
            if (value.loaded) {
                widgetInstances[value.id] = lightingControlInstanceModule(value.id, value['elementIds']);
            }
            else {
