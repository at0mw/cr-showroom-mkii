...
const shadeControlInstanceModule = (id, elementIds) => {
    'use strict';    

    // BEGIN::CHANGEAREA - your initialization code for each instance of widget goes here  
    // console.log(`shadeControl-widget shadeControlInstanceModule("${id}", [${elementIds}])`);
    ...
    const cleanup = () => {
        // console.log(`shadeControl-widget shadeControlInstanceModule cleanup("${id}")`);
    };
...
const shadeControlModule = (() => {
     ...
     */
    function onInit() {
       serviceModule.addEmulatorScenarioNoControlSystem("./app/project/components/widgets/shade-control/shade-control-emulator.json");
       // Uncomment the below line and comment the above to load the emulator all the time.
       // serviceModule.addEmulatorScenario("./app/project/components/widgets/shade-control/shade-control-emulator.json");
    }

    /**
     * private method for widget class creation
     */
    let loadedSubId = CrComLib.subscribeState('o', 'ch5-import-htmlsnippet:shadeControl-import-widget', (value) => {
        if (value['loaded']) {
            onInit();
            setTimeout(() => {
                CrComLib.unsubscribeState('o', 'ch5-import-htmlsnippet:shadeControl-import-page', loadedSubId);
                loadedSubId = '';
            });
        }
    });
     ...
     */
    CrComLib.subscribeState('o', 'ch5-template:shade-control-widget', (value) => {
        if (value['loaded'] !== undefined && value['id'] !== undefined) {
            if (value.loaded) {
                widgetInstances[value.id] = shadeControlInstanceModule(value.id, value['elementIds']);
            }
            else {
