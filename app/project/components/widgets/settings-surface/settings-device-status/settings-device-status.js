...
const settingsDeviceStatusInstanceModule = (id, elementIds) => {
    ...
    // console.log(`settingsDeviceStatus-widget settingsDeviceStatusInstanceModule("${id}", [${elementIds}])`);
    ...
    const cleanup = () => {
        // console.log(`settingsDeviceStatus-widget settingsDeviceStatusInstanceModule cleanup("${id}")`);
    };
...
const settingsDeviceStatusModule = (() => {
     ...
     */
    function onInit() {
       serviceModule.addEmulatorScenarioNoControlSystem("./app/project/components/widgets/settings-device-status/settings-device-status-emulator.json");
       // Uncomment the below line and comment the above to load the emulator all the time.
       // serviceModule.addEmulatorScenario("./app/project/components/widgets/settings-device-status/settings-device-status-emulator.json");
    }

    /**
     * private method for widget class creation
     */
    let loadedSubId = CrComLib.subscribeState('o', 'ch5-import-htmlsnippet:settingsDeviceStatus-import-widget', (value) => {
        if (value['loaded']) {
            onInit();
            setTimeout(() => {
                CrComLib.unsubscribeState('o', 'ch5-import-htmlsnippet:settingsDeviceStatus-import-page', loadedSubId);
                loadedSubId = '';
            });
        }
    });
